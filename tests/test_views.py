...
""" User Can Edit an Entry and Permissions are enforced"""
@pytest.mark.django_db
@pytest.mark.parametrize(
    "is_owner, expected_status",
    [
        (True, 302),   # owner can edit
        (False, 404),  # non-owner denied
    ],
)
def test_user_edit_entry_permissions(client, is_owner, expected_status):

    owner = UserFactory()
    other_user = UserFactory()

    entry = EntryFactory(topic__owner=owner)

    user = owner if is_owner else other_user
